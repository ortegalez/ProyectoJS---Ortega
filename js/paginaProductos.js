...
function mostrarProductos(productosElegidos) {
  if (contenedorProductos) {
    ...
    productosElegidos.forEach((producto) => {
      ...
      cardProducto.innerHTML = `<img class="producto-imagen" src="${producto.img}" alt="${producto.articulo}">
...
botonesCategorias.forEach((boton) => {
  boton.addEventListener("click", (e) => {
    ...
    if (e.currentTarget.id != "todos") {
      const nombreCategoria = BBDD.find(
        (producto) => producto.articulo === e.currentTarget.id
      );

      tituloPrincipal.innerText = nombreCategoria.categoria;

      const productosBoton = BBDD.filter(
        (producto) => producto.articulo === e.currentTarget.id
      );
